    ...
    <main id="main" class="main p-0">
        ...
        <div class="container mt-2" id="mainContent">
            ...
            <ul class="nav nav-tabs" id="questionTabs" role="tablist">
                ...
                <li class="nav-item">
                    <a class="nav-link" id="export-question-tab" data-toggle="tab" href="#export-question" role="tab"
                        aria-controls="add-question" aria-selected="false">Export Questions</a>
                </li>
            </ul>
            ...
            <div class="tab-content" id="questionTabsContent">
                ...
                <!-- Bulk Upload Section -->
                <div class="tab-pane fade" id="add-bulk-question" role="tabpanel" aria-labelledby="add-bulk-question-tab">
                    <div class="form-group">
                        <label for="quiz_id_bulk">Select Quiz</label>
                        <select id="quiz_id_bulk" name="quiz_id_bulk" class="form-control" required>
                            <option value="">Select Quiz</option>
                            <?php if (!empty($quizzes)): ?>
                                <?php foreach ($quizzes as $quiz): ?>
                                    <option value="<?= $quiz['quiz_id'] ?>"><?= $quiz['quiz_name'] ?></option>
                    ...
                    <form id="bulkUploadForm" enctype="multipart/form-data">
                        <div class="form-group">
                            <label for="questions_file">Upload Questions File (CSV)</label>
            ...
            <div class="modal fade" id="previewQuestionModal" tabindex="-1" aria-labelledby="previewQuestionModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div id="previewQuestionForm">
                            <div class="modal-header">
                                <h5 class="modal-title" id="previewQuestionModalLabel">Preview Question</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <input type="hidden" id="preview_question_id" name="question_id">
                               ...
                               <div>Correct Answer is Paris</div>
                ...
                axios.post('<?= base_url('admin/questionbank/bulkUpload') ?>', formData)
                    .then(response => {
                        if (response.data.success) {
                            ...
                            loadQuestions(); // Function to refresh or update questions list
                        } else {
...
//         How can a questions be by quiz, date, topic/category, difficult level and by question type
// How can that be done and will it be hard to implement?
// Cause I am using this quiz for my lms portal
...
$('#bulkUploadForm').submit(function (e) {
    e.preventDefault();
    let formData = new FormData(this);
    formData.append('quiz_id_bulk', $('#quiz_id_bulk').val());

    $.ajax({
        url: '<?= base_url("admin/questionbank/bulkUpload") ?>',
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
        success: function (response) {
            ...
            if (response.skipped_rows) {
                console.log('Skipped Rows:', response.skipped_rows);
            }
        },
