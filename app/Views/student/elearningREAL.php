...
<body>
    ...
    <script>
        ...
        //         return; // Exit the function if the CSRF token is not available
        //     }
        ...
        //             'X-CSRF-TOKEN': csrfToken // CodeIgniter uses this for CSRF validation
        //         },
        ...
        //         } else {
        //             console.error('Failed to mark module as completed.', data.message);
        //         }
        //     })
        ...
        fetch('<?= base_url('student/mark-module-completed') ?>', {
            ...
            body: JSON.stringify({ module_id: moduleId, course_id: courseId }) // Include course_id in payload
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
            ...
            $(document).ready(function () {
                ...
                $('#next-button').click(function () {
                    if (currentModuleIndex < modules.length - 1) {
                        ...
                        // Re-enable buttons after loading the module
                        setTimeout(() => {
                ...
                $('.module-button').click(function () {
                    // Mark the previous module as completed
                    const previousModuleId = $(modules[currentModuleIndex]).data('module-id');
                    markModuleAsCompleted(previousModuleId);

                    currentModuleIndex = $(this).data('index');
                    loadModule(currentModuleIndex);
                });

                // Add additional functionality for sidebar and progress circles if needed...
                $(document).ready(function () {
                ...
                document.querySelectorAll('.progress-circle').forEach(function (circle) {
                    ...
                    var color1 = percentage >= 25 ? '#f44336' : '#e0e0e0';
                    var color2 = percentage >= 50 ? '#ff9800' : '#e0e0e0';
                    var color3 = percentage >= 75 ? '#ffeb3b' : '#e0e0e0';
                    var color4 = percentage == 100 ? '#4caf50' : '#e0e0e0';
